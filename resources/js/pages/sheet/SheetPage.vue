            ...
            <div class="table-responsive mb-0">
              <table class="table table-hover table-list">
                ...
                <tr>
                  ...
                  <SheetTableHeader @sort="onFieldSorted" :key="f.name" v-for="f in fields" :field="f"/>
...
export default {
  name: "SheetPage",
  components: {SheetTableHeader, Icon, Action},
  data() {
    return {
      data: null,
      filters: this.$route.query.filters ? JSON.parse(atob(this.$route.query.filters)) : {},
      sorts: this.$route.query.sorts ? JSON.parse(atob(this.$route.query.sorts)) : {},
      query: {
  ...
  methods: {
    ...
    onFieldSorted(field, direction) {
      if (direction === null) {
        this.$delete(this.sorts, field.name)
      } else {
        this.$set(this.sorts, field.name, direction)
      }
    }
  },
  ...
  computed: {
    ...
    encodedSorts() {
      return this.sortsCount > 0 ? btoa(JSON.stringify(this.sorts)) : undefined
    },
    ...
    sortsCount() {
      return Object.keys(this.sorts).length
    },
